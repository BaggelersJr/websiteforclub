...
app.post("/rooms", requireAuth, (req, res) => {
    ...
    const insert = `INSERT INTO rooms (name, description, is_private, join_code, creator_id) VALUES (?, ?, ?, ?, ?)`;

    db.run(insert, [name.trim(), desc, is_private ? 1 : 0, joinCode, creatorId], function (err) {
        if (err) {
            if (err.message.includes("UNIQUE")) {
        ...
        db.run(`INSERT INTO room_members (user_id, room_id) VALUES (?, ?)`, [creatorId, roomId], (err2) => {
            if (err2) {
                return res.json({ success: false, message: "Room created but membership failed" });
            }
            res.json({ success: true, roomId, joinCode });
        });
    });
});
...
app.get("/my-rooms", requireAuth, (req, res) => {
    const sql = `
        SELECT r.id, r.name, r.description, r.created_at, u.username AS creator, COUNT(rm2.user_id) AS member_count
        FROM rooms r
        JOIN users u ON r.creator_id = u.id
...
app.post("/rooms/join-code", requireAuth, (req, res) => {
    ...
    db.get(findRoom, [code], (err, room) => {
        if (err || !room) {
            return res.json({ success: false, message: "Invalid join code" });
        }
...
app.post("/rooms/:id/messages", requireAuth, (req, res) => {
    ...
    db.get(checkmem, [userId, roomId], (err, membership) => {
        ...
        db.run(insert, [roomId, userId, content.trim()], function (err2) {
            ...
            db.get(`SELECT username FROM users WHERE id = ?`, [userId], (err3, user) => {
                if (err3 || !user) {
                    return res.json({ success: true, messageId: this.lastID, author: "Unknown" });
                }
                res.json({ success: true, messageId: this.lastID, author: user.username });
            });
        });
    });
});
...
app.get("/rooms/:id/messages", requireAuth, (req, res) => {
    ...
    db.get(`SELECT * FROM rooms WHERE id = ?`, [roomId], (err, room) => {
        ...
        db.get(`SELECT id FROM room_members WHERE user_id = ? AND room_id = ?`, [userId, roomId], (err2, membership) => {
            if (err2 || !membership) {
            ...
            db.all(messagesSql, [roomId], (err3, messages) => {
                ...
                res.json({ success: true, room, currentUserId: userId, messages });
            });
        });
    });
});
...
app.get("/rooms/:id/settings", requireAuth, (req, res) => {
    const roomId = parseInt(req.params.id);
    ...
    db.get(sql, [roomId], (err, room) => {
        ...
        res.json({ success: true, room, currentUserId: req.session.userId });
    });
});
...
app.post("/rooms/:id/edit", requireAuth, (req, res) => {
    ...
    db.get(`SELECT join_code FROM rooms WHERE id = ? AND creator_id = ?`, [roomId, userId], (err, room) => {
        if (err || !room) {
            return res.json({ success: false, message: "Room not found or you are not the owner" });
        }
        const joinCode = is_private ? (room.join_code || generateJoinCode()) : null;

        const sql = `
            UPDATE rooms
            SET name = ?, description = ?, is_private = ?, join_code = ?
            WHERE id = ? AND creator_id = ?
        `;
        db.run(sql, [name.trim(), description || null, is_private ? 1 : 0, joinCode, roomId, userId], function (err2) {
            if (err2) {
